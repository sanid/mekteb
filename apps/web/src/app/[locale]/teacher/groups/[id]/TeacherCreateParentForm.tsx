...
export function TeacherCreateParentForm({
  action,
  students,
  baseUrl,
  qrEnabled,
  ...
  students: Array<{ enrollmentId: string; studentId: string; name: string }>;
  ...
  if (state?.ok) {
    return (
      <div className="space-y-3 rounded-lg border border-accent/40 bg-accent-subtle p-4">
        <p className="text-sm font-medium">{t("parentAccountCreated")}</p>
        <p className="text-sm">{t("shareTempPassword")} {state.full_name} ({state.email}). {t("forcedToChange")}</p>
  ...
  return (
    <form action={formAction} className="space-y-3">
      <label className="block space-y-1">
        <span className="text-sm font-medium">{tT("linkToStudent")}</span>
        <select
      ...
      >
        {pending ? t("creating") : tT("addParent")}
      </button>
    </form>
  );
}
