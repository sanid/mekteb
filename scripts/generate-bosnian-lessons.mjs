...
async function generateMigration() {
  ...
  sql.push(`-- Migration: Bosnian Ilmihal 1 curriculum lessons extracted from book images
...
begin
  for r_mosque in select id from public.mosques loop
    -- Resolve a creator user id for the mosque if available
    select user_id into v_admin_id
    from public.memberships
    where mosque_id = r_mosque.id and role = 'mosque_admin'
    limit 1;

    -- 1. Insert or get Topic 1
...
`);
  ...
  for (const lesson of lessons) {
    const topicVar =
      lesson.topic_key === 'topic_1'
        ? 'v_topic1_id'
        : lesson.topic_key === 'topic_2'
        ? 'v_topic2_id'
        : 'v_topic3_id';

    const escapedTitle = lesson.title.replace(/'/g, "''");
    const jsonStr = JSON.stringify(lesson.blocks).replace(/'/g, "''");

    sql.push(`    -- Lesson ${lesson.sort_order}: ${lesson.title}
    if exists (
      select 1 from public.lessons
      where mosque_id = r_mosque.id and title = '${escapedTitle}'
    ) then
      update public.lessons
      set topic_id = ${topicVar},
          body = '${jsonStr}'::jsonb,
